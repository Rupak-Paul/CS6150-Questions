...
int main() {
    ...
    vector<vector<pair<int, int>>> minDistanceNeighbour(n, vector<pair<int, int>>(m, {-1, -1}));
    vector<vector<bool>> visited(n, vector<bool>(m, false));
    queue<pair<int, int>> q;
    
    minDistanceFromX[currentLocation.first][currentLocation.second] = 0;
    minDistanceNeighbour[currentLocation.first][currentLocation.second] = currentLocation;
    ...
    while(!q.empty()) {
        ...
        for(pair<int, int> v : adj) {
            ...
            minDistanceNeighbour[v.first][v.second] = u;
    ...
    if(distNearestHospital == INT_MAX) cout << "-1" << endl;
        ...
        for(pair<int, int> u = locNearestHospital; u != currentLocation; u = minDistanceNeighbour[u.first][u.second]) {
            path.push_back(u);
        }
        path.push_back(currentLocation);
        reverse(path.begin(), path.end());

        for(pair<int, int> u : path) {
